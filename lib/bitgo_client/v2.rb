...
module BitgoClient
  class V2
    ...
    def create_address(wallet_id, coin_code: :tbtc, logger: nil, proxy: nil)
      client.request("#{base_path}/#{coin_code}/wallet/#{wallet_id}/address", method: :post, logger: logger, proxy: proxy)
    end

    def address(wallet_id, address, coin_code: :tbtc, logger: nil, proxy: nil)
      client.request("#{base_path}/#{coin_code}/wallet/#{wallet_id}/address/#{address}", logger: logger, proxy: proxy)
    end

    def fee(coin_code: :tbtc, logger: nil, proxy: nil)
      client.request("#{base_path}/#{coin_code}/tx/fee", logger: logger, proxy: proxy)
    end

    def get_transfer(wallet_id, transfer_id, coin_code: :tbtc, logger: nil, proxy: nil)
      client.request("#{base_path}/#{coin_code}/wallet/#{wallet_id}/transfer/#{transfer_id}", logger: logger, proxy: proxy)
    end
    ...
    def transactions(wallet_id, coin_code: :tbtc, logger: nil, limit: 25, prev_id: nil, all_tokens: nil, proxy: nil)
      ...
      client.request("#{base_path}/#{coin_code}/wallet/#{wallet_id}/tx?#{query_string}", logger: logger, proxy: proxy)
    end

    def transaction(wallet_id, transaction_id, coin_code: :tbtc, logger: nil, proxy: nil)
      client.request("#{base_path}/#{coin_code}/wallet/#{wallet_id}/tx/#{transaction_id}", logger: logger, proxy: proxy)
    end
    ...
    def transfers(wallet_id, coin_code: :tbtc, logger: nil, limit: 25, prev_id: nil, all_tokens: nil, proxy: nil)
      ...
      client.request("#{base_path}/#{coin_code}/wallet/#{wallet_id}/transfer?#{query_string}", logger: logger, proxy: proxy)
    end
    ...
    def wallet(wallet_id, coin_code: :tbtc, logger: nil, all_tokens: nil, proxy: nil)
      ...
      client.request("#{base_path}/#{coin_code}/wallet/#{wallet_id}?#{query_string}", logger: logger, proxy: proxy)
    end
